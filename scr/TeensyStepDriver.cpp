...
void StepMotor::smoothStop(bool smooth){
	_smoothStp = smooth;
}
...
void StepMotor::_stepISR(StepMotor* self){
	...
	bool smoothStop = self->_smoothStp;
	...
	const float dStop = (absVel*absVel) / (2.0f*self->_accel); // Requiered distance to accelerate or decelerate
	...
	if (stopRequest || (!continuous && absRemaining <= dStop)){
		// Decelerate
		if (smoothStop){  //Deceleration ramp
			temp_newSpeed = absVel - self->_accel*dt;
			newSpeed = dirSign*(temp_newSpeed > MIN_SPEED_SPS ? temp_newSpeed : MIN_SPEED_SPS);
		}
		else{ // sudent stop when target is reached
			newSpeed = dirSign*absVel; // keeps full speed until the traget is reached
		}
	}
